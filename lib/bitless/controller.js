...
Server.prototype.listen = function(port, callback) {
	...
	port = port === undefined ? utils.array.pick(PORTS) : port;
	callback = callback || noop;
	...
	server.on('error', function(err) {
		logger.error('Server failed:', err.stack);
...
var Controller = common.emitter(function(options) {
	utils.arguments.expect(options, 'torrent', 'tracker', 'storage', 'server', 'id', 'port');
...
exports.create = function(torrent, options) {
	...
	var server = options.server || new Server();
	var port = server.port || options.port || utils.array.pick(PORTS);
