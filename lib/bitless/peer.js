...
var Peer = common.emitter(function(torrent, socket, addr) {
	...
	messenger.on('message', function(message) {
		...
		if(!self.isReady() && id !== protocol.Bitfield.ID) {
			self.haves = new bitfield.Bitfield(torrent.numberOfPieces);
		}
		...
		try {
			...
			case protocol.Bitfield.ID:
				...
				message = protocol.Bitfield.unpack(message, torrent.numberOfPieces);
...
exports.acceptHandshake = function() {};
exports.initiateHandshake = function(options, callback) {
	...
	var connection = socket.connect(options.address.port, options.address.ip);
