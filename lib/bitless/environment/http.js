...
URL.parse = function(url, base) {
	...
	var protocol = url.protocol || (base.protocol || '').replace(/:$/, '') || 'http';
	var port = url.port || base.port || (protocol === 'http' ? 80 : 443);
	var hostname = url.host || base.hostname;

	var query = {};

	(url.query || '').split('&').filter(function(pair) {
		return !!pair;
